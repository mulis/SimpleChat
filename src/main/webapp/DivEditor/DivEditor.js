  ...
  editPanel.element.on(
    ...
    function(aEvent) {
      if (aEvent.which != 1) {
        ...
        linkEdit.putProperties({text : aEvent.target.href});
        linkEdit.refresh();
      }
    }
  );
...
DivEditor.prototype.applyStyle = function(appllier) {
  ...
  if (selection.rangeCount) {
    ...
    if (!range.collapsed) {
      ...
      if (parent.hasClass('divEditor-textEdit') || parent.parents('.divEditor-textEdit').length > 0) {
...
//      if (parent.hasClass('divEditor-textEdit')) {
